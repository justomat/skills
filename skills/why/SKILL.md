...
Before spawning investigators, list the available MCPs from the agent environment (`.agents`, `.claude`, or Cursor). Use the available-tools map when present. Otherwise inspect configured MCP servers or MCP directories exposed in the environment.
