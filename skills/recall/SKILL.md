...
Transcripts typically live in the workspace's `agent-transcripts/` directory, or in agent project stores such as `~/.claude/projects/`, `~/.agents/projects/`, or legacy `~/.cursor/projects/<slug>/agent-transcripts/<uuid>/<uuid>.jsonl` (where `<slug>` is the workspace path with the leading slash dropped and each "/" turned into "-", so `/Users/you/proj` becomes `Users-you-proj`). Every line is one chat message.
